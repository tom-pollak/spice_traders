package com.mygdx.pirategame.pathfinding.pathfinding.grid.finders;
...
/**
 * Optimization over A*. This will always use @{link {@link EuclideanDistance}, regardless of the
 * one set in the passed {@link #options}. This should only be used on {@link NavigationGridGraph}
 *
...
public class JumpPointFinder<T extends NavigationGridGraphNode> implements PathFinder<T> {
  ...
  /**
   * Find and return the path. The resulting collection should never be modified, copy the values
   * instead.
   *
  ...
  private void identifySuccesors(T node, NavigationGridGraph<T> graph, int job, T start, T end) {
    ...
    for (T neighbor : neightbors) {
      ...
      boolean isDiagonalJump =
          (jumpPoint.getX() != node.getX()) && (jumpPoint.getY() != node.getY());
      if (isDiagonalJump && !options.allowDiagonal) continue;
    ...
    if (parent != null) {
      ...
      if (dx != 0 && dy != 0) {
        ...
        if (dx == 0) { // on y
          ...
          if (!allowDiagonal) {
            if (grid.isWalkable(x + 1, y)) neighbors.add(grid.getCell(x + 1, y));
            if (grid.isWalkable(x - 1, y)) neighbors.add(grid.getCell(x - 1, y));
          }
          ...
          // In case diagonal moves are forbidden
          if (!allowDiagonal) {
            if (grid.isWalkable(x, y + 1)) neighbors.add(grid.getCell(x, y + 1));
            if (grid.isWalkable(x, y - 1)) neighbors.add(grid.getCell(x, y - 1));
          }
        }
      }
  ...
  private T jump(T node, T parent, NavigationGridGraph<T> grid, T start, T end) {
    ...
    if (grid.isWalkable(x + dx, y + dy)) {
      ...
      if (nextJump != null) return nextJump;
    }

    // if cant keep going on a straight line, try a 90 degrees turn
    if (!allowDiagonal) {
      if (dx == 0 && (grid.isWalkable(x, y + 1) || grid.isWalkable(x, y - 1))) return node;

      // diagonals are forbidden
      if (dy == 0 && (grid.isWalkable(x + 1, y) || grid.isWalkable(x - 1, y))) return node;
    }
  ...
  boolean allowedDiagonalMovement(T node, int dx, int dy, NavigationGridGraph<T> grid) {
    if (options.allowDiagonal) {
      if (!options.dontCrossCorners) return true;

      if (dx != 0 && dy != 0) return true;

      if (dx == 0) {
        return (grid.isWalkable(node.getX() + 1, node.getY())
                || grid.isWalkable(node.getX() - 1, node.getY()))
            && grid.isWalkable(node.getX(), node.getY() + dy);
      }

      return (grid.isWalkable(node.getX(), node.getY() + 1)
              || grid.isWalkable(node.getX(), node.getY() - 1))
