...
public class JumpPointFinder<T extends NavigationGridGraphNode> implements PathFinder<T> {
    ...
    private void identifySuccesors(T node, NavigationGridGraph<T> graph, int job, T start, T end) {
        ...
        for (T neighbor : neightbors) {
            ...
            boolean isDiagonalJump = (jumpPoint.getX() != node.getX()) && (jumpPoint.getY() != node.getY());
    ...
    private T jump(T node, T parent, NavigationGridGraph<T> grid, T start, T end) {
        ...
        if (dx != 0 && dy != 0) {
            if ((grid.isWalkable(x - dx, y + dy) && !grid.isWalkable(x - dx, y)) || (grid.isWalkable(x + dx, y - dy) && !grid.isWalkable(x, y - dy))) {
                return node;
            }
            ...
            if (dx != 0) { // moving along x
                if (allowDiagonal) {
                    if ((grid.isWalkable(x + dx, y + 1) && !grid.isWalkable(x, y + 1)) || (grid.isWalkable(x + dx, y - 1) && !grid.isWalkable(x, y - 1))) {
                        return node;
                    }
                }
            } else { // moving along y
                if (allowDiagonal) {
                    if ((grid.isWalkable(x + 1, y + dy) && !grid.isWalkable(x + 1, y)) || (grid.isWalkable(x - 1, y + dy) && !grid.isWalkable(x - 1, y))) {
                        return node;
                    }
                }
            }
