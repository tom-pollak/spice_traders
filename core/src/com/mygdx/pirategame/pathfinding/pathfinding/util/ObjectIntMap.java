...
package com.mygdx.pirategame.pathfinding.pathfinding.util;
...
/**
 * An unordered map where the values are ints. This implementation is a cuckoo hash map using 3
 * hashes, random walking, and a small stash for problematic keys. Null keys are not allowed. No
 * allocation is done except when growing the table size. <br>
 * <br>
 * This map performs very fast get, containsKey, and remove (typically O(1), worst case O(log(n))).
 * Put may be a bit slower, depending on hash collisions. Load factors greater than 0.91 greatly
 * increase the chances the map will have to rehash to the next higher POT size.
 *
 * @author Nathan Sweet
 */
public class ObjectIntMap<K> implements Iterable<ObjectIntMap.Entry<K>> {
  ...
  private void push(
      K insertKey, int insertValue, int index1, K key1, int index2, K key2, int index3, K key3) {
    K[] keyTable = this.keyTable;
   ...
   * defaultValue + increment is put into the map.
   */
  ...
  private boolean containsKeyStash(K key) {
    K[] keyTable = this.keyTable;
    for (int i = capacity, n = i + stashSize; i < n; i++) if (key.equals(keyTable[i])) return true;
    return false;
  }
  ...
  /**
   * Returns an iterator for the entries in the map. Remove is supported. Note that the same
   * iterator instance is returned each time this method is called. Use the {@link Entries}
  ...
  /**
   * Returns an iterator for the values in the map. Remove is supported. Note that the same iterator
   * instance is returned each time this method is called. Use the {@link Entries} constructor for
  ...
  /**
   * Returns an iterator for the keys in the map. Remove is supported. Note that the same iterator
   * instance is returned each time this method is called. Use the {@link Entries} constructor for
   * nested or multithreaded iteration.
   */
  public Keys<K> keys() {
